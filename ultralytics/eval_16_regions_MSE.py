...
output_path = "test_eval_pruned"

os.makedirs(output_path, exist_ok=True)

for region_id in regions:
    # Construct data_path and name for each model based on region ID
    data_path = f"../../16_Regions/datasets/{region_id}_top_salient"
    name = f"yolov8s_custom_tuned_{region_id}_top_salient"
    ...
    with open(filename, mode='w', newline='') as file:
        ...
        for i in range(len(avg_class_mse.keys())):
            ...
            #print("class: ", list(avg_class_mse.keys())[i], list(avg_class_err.keys())[i])
